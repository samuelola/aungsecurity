                                ...
                                <div class="tab-content dark-field shipping-content" id="cart-options-tabContent">
                                  <div class="tab-pane fade show active" id="bio" role="tabpanel" aria-labelledby="bio-tab">
                                    ...
                                    <form method="post" action="{{route('adminkyc.bio',$tenant->subdomain)}}" class="row g-3 needs-validation basic-form" id="bioForm">
                                      ...
                                      <div class="col-sm-4">
                                        <label class="form-label" for="customEmail01">Gender<span class="text-danger">*</span></label>
                                        
                                        <div style="color:#dc3545;" class="valid-feedback">Looks good!</div>
                                        <select  name="gender" class="form-select">
                                            
                                            <option value="">Select Gender</option>
                                                <option value="male" {{ $kyc?->gender == 'male' ? 'selected' : '' }}>
                                                    Male
                                                </option>
                                                <option value="female" {{ $kyc?->gender == 'female' ? 'selected' : '' }}>
                                                    Female
                                                </option>
        ...
        error: function (xhr) {
            ...
            if (xhr.status === 422 && xhr.responseJSON.errors) {
                ...
                $.each(xhr.responseJSON.errors, function (field, messages) {
                    ...
                    input.after(`
                        <div style="color:#dc3545;" class="invalid-feedback">
                            ${messages[0]}
                        </div>
                    `);
                });
            ...
            // Database duplicate / custom errors
...
$('#docForm').on('submit', function (e) {
    ...
    let fileInput = $('input[name="id_document"]')[0];

    let file = fileInput.files[0];

    // 1MB = 1048576 bytes
    let maxSize = 1048 * 1024;
    ...
    if (file && file.size > maxSize) {
        ...
        $('input[name="id_document"]')
            .addClass('is-invalid')
            .after(`
                <div class="invalid-feedback d-block" style="color:#dc3545;">
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (!response.doc_already_completed) {
            ...
            new bootstrap.Tab(
                document.querySelector('#facee-tab')
            ).show();
        },
