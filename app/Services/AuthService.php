...
class AuthService implements RegisterInterface{
   ...
   public function generateVerificationCode($user){
    ...
    // Queue email
     Mail::to($user->email)->send(new TenantResetPasswordMail($user));
    ...
    public function addWallet($user_id,$tenant){
        ...
        if(!DB::table('resident_wallets')->where('user_id',$user_id)->exists()){
           ...
           $userwallets = $currencies->map(function ($currency) use ($user_id,$tenant){
