...
class SubscriptionService
{
...

    {
       ...
       // check subaccount
        ...
        if (!$tenant_subaccount || !$tenant_subaccount->subaccount_code) {
        ...
        $resident_wallet = Wallet::where('user_id', auth()->id())
                           ->where('tenant_id', $tenant->id)->first();

        if($resident_wallet->balance < $new_total){
            
             throw new \Exception("Amount in wallet too low for this subscription.");
        }


        // wallet deduction 

        DB::transaction(function () use ($new_total) {
             $user_id = auth()->user()->id;
             $from_user_wallet = Wallet::with('user')->lockForUpdate()->where('user_id',$user_id)->first();
             $st_amount = $new_total;
             $from_user_wallet->update(['balance' => $from_user_wallet->balance - $st_amount]);
...
public function verifyPaymentAndCreateSubscription($reference)
{
    ...
    $user_auth_code = DB::table('user_auto_code')
    ...
    Subscription::create([
        ...
        'ends_at' => $end,
        'status' => 'active',
        'authorization_code' => $user_auth_code->auth_code,
